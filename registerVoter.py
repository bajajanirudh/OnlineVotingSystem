...
def Register(root,frame1):
    ...
    root.title("Register New Voter")
    for widget in frame1.winfo_children():
        widget.destroy()

    Label(frame1, text="Register New Voter", font=('Helvetica', 18, 'bold')).grid(row = 0, column = 2, rowspan=1)
    ...
    Label(frame1, text="Metamask Blockchain Address:             ", anchor="e", justify=LEFT).grid(row = 6,column = 0)
    ...
    e4['values'] = ("Male","Female","Prefer not to say")
